import socket,json,hashlib,asyncio
...
class Client():
	...
	async def recvMessage(self):
		while not getattr(self.socket, '_closed') and not self.close:
	...
	async def listenSocket(self):
		while not self.close:
			while not self.tryConnectToServer("127.0.0.1",11451):
				pass
			print("连接成功")
			self.recvMessage()
...
asyncio.run(run())
# 我也不知道能不能成功运行 这破电脑只能装py3.6 我运行不了