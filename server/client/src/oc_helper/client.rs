...
use pb::auth::v1::auth_service_client::AuthServiceClient;
use pb::basic::v1::basic_service_client::BasicServiceClient;
...
impl TestApp {
    ...
    /// # Panics
    ///
    /// Panics if launching with a existing instance.
    pub async fn get_db_connection(&self) -> &sea_orm::DatabaseConnection {
        &self.db_pool.as_ref().unwrap().db_pool
    }

    pub async fn check_ban_status(&self, user: ID, session_id: SessionID) -> Option<Duration> {
        todo!()
    }

    pub async fn check_mute_status(&self, user: ID, session_id: SessionID) -> Option<Duration> {
        todo!()
    }
}
