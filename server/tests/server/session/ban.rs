// TODO:check the influence of banned user
use claims::{assert_err, assert_gt, assert_none};
use client::TestApp;
...
async fn ban_user_with_duration() {
    ...
    assert_none!(app.check_ban_status(bid, session.session_id).await);
    assert_none!(app.check_ban_status(cid, session.session_id).await);
    app.async_drop().await;
}
...
async fn ban_already_banned_user() {
    ...
    a.lock()
        ...
        .await
        .unwrap();
    let rest = app.check_ban_status(bid, session.session_id).await;
    assert_gt!(rest.unwrap().as_secs(), 10 - 3);
    app.async_drop().await;
}
