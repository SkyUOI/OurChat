...
pub fn redis_hset_derive(input: TokenStream) -> TokenStream {
    ...
    let field_name_strs: Vec<String> = fields.iter().map(|field| field.ident.as_ref().unwrap().to_string()).collect();

    // Generate HSET calls for writing
    let hset_calls = field_idents.iter().zip(field_name_strs.iter()).map(|(ident, name_str)| {
        quote! {
